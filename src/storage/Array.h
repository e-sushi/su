...
struct Array {
    ...
    static Array<T> create(u32 initial_space = 4);

	void init(u32 initial_space = 4);
    ...
    void insert(spt idx, const T& val);
    ...
    // 'idx' are moved left
    void remove(u32 idx, b32 unordered = false);
    ...
    // and returns a new Array with those contents
    Array<T> copy(u64 start, u64 count);

    T  operator[](s64 i) const { return read(i); }
	T& operator[](s64 i)       { return readref(i); }
};
